...
test('<WikiInsight wiktionary>', async () => {
  const { getAllByText, getByText } = render(
    <WikiInsight wiktionary doc={wtf(testWikitext)} />
  );

  // Validate all sections were rendered
  getByText("McCarthy's fifth book", { exact: false });
  getByText('The novel follows an adolescent runaway', { exact: false });
  getByText('historical novel');

  // Validate attribution
  let el = getByText('Source: (English) Wiktionary.org:');
  el = el.querySelector('a')!;
  expect(el.textContent).toBe(
    'Blood Meridian or The Evening Redness in the West'
  );
  expect((el as HTMLAnchorElement).href).toBe(
    'https://en.wiktionary.org/wiki/Blood%20Meridian%20or%20The%20Evening%20Redness%20in%20the%20West'
  );
  ...
  // Click "Cormac McCarthy" link
  fireEvent.click(getAllByText('Cormac McCarthy')[0]);

  // Validate new article was loaded
  await waitForDomChange();
  expect(mockFetch).toHaveBeenCalledTimes(3);
  expect(mockFetch.mock.calls[0][0]).toBeUndefined(); // no innerText in jsdom
  expect(mockFetch.mock.calls[1][0]).toBe('Cormac_McCarthy');
  expect(mockFetch.mock.calls[2][0]).toBe('cormac_mccarthy');
  getByText('American novelist, playwright, and', { exact: false });
});
